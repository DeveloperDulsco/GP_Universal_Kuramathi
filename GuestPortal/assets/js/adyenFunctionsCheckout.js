...
// Prefer layout/view not loading this file; this is a safety net if it loads without the flag set early
var __paymentDisabled = (typeof IsPaymentDisabled !== 'undefined') && (IsPaymentDisabled === true || IsPaymentDisabled === 'true' || IsPaymentDisabled === 'True');
if (__paymentDisabled) {
                    ...
                    if (response.result) {
                        if (response.isRedirect) {
                            ...
                            InsertPaymentHeaderDetails(response.adyenResponse.ResponseObject, ReservationNumber, transactionID, ReservationNameID, transactionType).then(saveresponse => {
                                ...
                                $('#paymentStatusMessage').html(language == "fr" ? "Votre paiement a été effectué avec succès." : "Your payment was successful.");
