    ...
    getPaymentMethods().then(paymentMethodsResponse => {
        ...
        const translations = {
            "en-US": {
                ...
                "payButton": language != null && language == "fr" ? "Valider" : "Authorise",
                ...
                getCostEstimater(costestimatorPostData).then(estimatorResponse => {
                    ...
                    CalculatePreAuthAmount(data).then(preauthAmountResponse => {
                        if (preauthAmountResponse.RoomRate > 0) {
                            $('#spanRoomRate').html("EUR " + preauthAmountResponse.RoomRate);
                        }
                        else {
                            $('#spanRoomRate').html("--");
                        }
                        if (preauthAmountResponse.IncidentialCharge > 0) {
                            $('#spanIncidential').html("EUR " + preauthAmountResponse.IncidentialCharge + " x " + preauthAmountResponse.NoofNights + " Night(s)");
                        }
                        else {
                            $('#spanIncidential').html("--");
                        }
                        $('#spanTotalAmount').html("EUR " + preauthAmountResponse.PreuthAmount);

                        if (transactionType == "Sale") {
                            $('#idPaymentTitleLable').html(language == "fr" ? "DÉTAILS DE L'AUTORISATION" : "AUTHORISATION DETAILS");
                            $('#paymentAmountConfirmationModalLabel').html(language == "fr" ? "Veuillez vérifier les détails de votre autorisation" : "Please verify your authorisation details")
                        }
                        else {
                            $('#idPaymentTitleLable').html(language == "fr" ? "DÉTAILS DE L'AUTORISATION" : "AUTHORISATION DETAILS");
                            $('#paymentAmountConfirmationModalLabel').html(language == "fr" ? "Veuillez vérifier les détails de votre autorisation" : "Please verify your authorisation details");
            ...
            if (response.isRedirect) {
                ...
                InsertPaymentHeaderDetails(response.adyenResponse.ResponseObject, ReservationNumber, transactionID, ReservationNameID, transactionType).then(saveresponse => {
                    if (saveresponse.result) {
                        ...
                        $('#paymentStatusMessage').html(language == "fr" ? "Votre autorisation a réussi." : "Your authorisation was successful.");
